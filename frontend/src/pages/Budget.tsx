...
export default function Budget() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([api.categories(selectedUser), api.getBudget(selectedUser)])
      ...
      .catch((err) => {
        console.error('[Budget] Failed to load data:', err);
        setError('Unable to load budget data. The server may be warming up — please try again.');
      });
  }, [selectedUser]);
  ...
  const handleSave = async (cat: string) => {
    ...
    try {
      ...
      console.error('[Budget] Failed to save budget:', err);
    }
    setSaving(null);
  };
      ...
      <div className="page-body">
        {error ? (
          <div className="error-state">
            ...
            <button className="btn btn-primary" onClick={load} id="budget-retry">Retry</button>
          </div>
        ) : (
          <>
            {/* Summary cards */}
